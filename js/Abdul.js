...
function placeOrder(restaurantId, items, address) { 
    ...
    // Validar que todos los items existen en el menú del restaurante
    const invalidItems = items.filter(item => !restaurant.menu.includes(item));
    if (invalidItems.length > 0) {
        console.log(`Los siguientes items no están en el menú: ${invalidItems.join(", ")}`);
        return;
    }
...
function showOrders() { 
    ...
    orders.forEach(order => { 
        console.log(`ID: ${order.id}, Restaurante: ${order.restaurant}, Items: ${order.items.join(", ")}, Dirección: ${order.address}, Estado: ${order.status}`); 
    }); 
} 
